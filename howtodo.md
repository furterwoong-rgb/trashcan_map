...
| 우선순위 | 작업 항목 | 작업 성격 | 예상 소요 | 기대 효과 |
| :--- | :--- | :---: | :---: | :--- |
| **P0 (즉시 구현)** | **1. 모바일 지도 해상도 고급화 (@2x Retina)** | 프론트엔드 UI | 10분 | 즉각적인 시각적 퀄리티/고급화 체감 |
| **P0 (즉시 구현)** | **2. 퍼널 전환율 추적 (Vercel Custom Events)** | 데이터/통계 | 30분 | 유저들이 길찾기까지 얼마나 도달하는지 실시간 측정 |
| **P1 (핵심 고도화)**| **3. 실시간 위치 매핑 (watchPosition & 락인)** | 핵심 기능 | 2시간 | 실제 네이버지도처럼 내 위치가 실시간으로 따라옴 |
| **P1 (핵심 고도화)**| **4. 위치 불안정 안내 가이드라인 모달** | UX/CS 대응 | 1시간 | "위치 안 맞아요" 불만 원천 차단 및 이탈 방지 |
| **P2 (크라우드소싱)**| **5. 가득 찬 정도 간이 투표 & 신규 제보 받기** | 사용자 참여 | 2시간 | 서버 없이 구글시트/슬랙 연동으로 데이터 수집 시작 |
| **P3 (AI 기능)**   | **6. Llama / Gemini Vision 기반 쓰레기통 AI 인증** | AI/백엔드 | 3~4시간 | 사진 판별을 통한 허위 제보 자동 필터링 |
